...
use crate::{Actionable, Contextualized, Dynamic, Frame, Status};

/// Extension trait for `Result<T, E>` to add context.
pub trait ResultExt<T, E> {
    /// Wrap the error in [`Contextualized`] and add context.
    ///
    /// # Errors
    ///
    /// Returns the original error wrapped in [`Contextualized`] with the given context frame.
    fn wrap_ctx(self, message: impl Into<String>) -> Result<T, Contextualized<E, Dynamic>>
    where
        E: Actionable;
    ...
    /// # Errors
    ///
    /// Returns the original error wrapped in [`Contextualized`] with a lazily-built context frame.
    fn wrap_ctx_lazy<F: FnOnce() -> String>(self, f: F) -> Result<T, Contextualized<E, Dynamic>>
    where
        E: Actionable;
    ...
    fn wrap_ctx_bounded<Overflow: Spout<Frame, Error = core::convert::Infallible>>(
        ...
        overflow: Overflow,
        max_frames: usize,
    ) -> Result<T, Contextualized<E, Dynamic, Overflow>>
    where
        E: Actionable;
}
...
impl<T, E> ResultExt<T, E> for Result<T, E> {
        ...
        E: Actionable,
    {
        ...
        self.map_err(|e| Contextualized::new(e).with_frame(frame))
    }
        ...
        E: Actionable,
    {
        ...
        self.map_err(|e| {
            ...
            Contextualized::new(e).with_frame(frame)
        })
    }
    ...
    {
        let frame = Frame::here(message);
        self.map_err(|e| Contextualized::with_overflow(e, overflow, max_frames).with_frame(frame))
    }
}
...
pub trait ContextExt<T, E, S: Status, Overflow: Spout<Frame, Error = core::convert::Infallible>> {
    ...
    fn with_ctx_lazy<F: FnOnce() -> String>(
        self,
        f: F,
...
pub trait OptionExt<T> {
    ...
    /// # Errors
    ///
    /// Returns a [`Contextualized`] error with the given context frame when `None`.
    fn wrap_ctx<E: Actionable>(
        self,
        error: E,
        message: impl Into<String>,
    ) -> Result<T, Contextualized<E, Dynamic>>;

    /// Convert `None` into a contextualized error with lazy evaluation.
    ///
    /// # Errors
    ///
    /// Returns a [`Contextualized`] error with a lazily-built context frame when `None`.
...
impl<T> OptionExt<T> for Option<T> {
    ...
    fn wrap_ctx<E: Actionable>(
        ...
        match self {
            ...
            None => {
                let frame = Frame::here(message);
                Err(Contextualized::new(error).with_frame(frame))
            }
        }
    }
...
pub trait IntoContextualized<E> {
    /// Convert into a contextualized error.
    fn into_ctx(self) -> Contextualized<E, Dynamic, DropSpout>
    where
        E: Actionable;
}

impl<E> IntoContextualized<E> for E {
    fn into_ctx(self) -> Contextualized<E, Dynamic, DropSpout>
    where
        E: Actionable,
    {
        Contextualized::new(self)
    }
}
