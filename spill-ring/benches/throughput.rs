...
//!
//! All rings are created once (pre-warmed) and reset via `clear()` between
//! iterations. This measures steady-state throughput, not warming overhead.
...
///
/// Rings are pre-warmed and reused across iterations via `clear()`.
...
///
/// NOTE: CollectSpout accumulates items, so the ring + sink must be recreated
/// each iteration. The warming cost is included in measurement here.
///
/// API improvement opportunity: a `reset()` method on SpillRing that clears the
/// ring AND resets the sink (via a `Reset` trait on Spout) would let us hoist
/// these outside the loop too.
...
///
/// Rings are pre-warmed. Each iteration refills the ring and then pops all items.
...
///
/// Rings are pre-warmed and reset between iterations.
